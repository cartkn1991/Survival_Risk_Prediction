#!/usr/bin/env python3
"""Resume downstream pipeline after feature-importance aux (skip completed steps)."""
import argparse
import subprocess
import sys
...
align = fi_root / "alignment"

steps = [
    f"{PY} select_significant_features.py {JOINT} --out-dir {sig_out} --W-npy {fi_root}/dann_W.npy --alignment-dir {align} --device {args.device}",
    ...
    f"{PY} bio_relevance/epigenetic_clock_mortality_benchmark.py {JOINT} --out-dir {clocks_out} --device cuda",
...
for cmd in steps:
    print(f"\n>>> {cmd}\n", flush=True)
    code = subprocess.call(cmd, shell=True, cwd=str(ROOT))
    if code != 0:
        print(f"FAILED: {cmd}")
        sys.exit(code)
